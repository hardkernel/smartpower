...
char RcvUartBuf[4];
char TransUartBuf[40];
unsigned char g_btready=0;
unsigned char g_bt_init=0;
...
void YourHighPriorityISRCode()
{
	...
	if(PIR1bits.RCIF)
	{
		memset((void*)&TransUartBuf[0],0x00,sizeof(TransUartBuf));
		gets1USART(RcvUartBuf, 2);
		switch(RcvUartBuf[0]) {
			case 'O' : 
				if(RcvUartBuf[1]=='K') g_btready=1;
				break;
            case 0x31:  //Toggle On/Off
            	if((g_btready)&&(RcvUartBuf[1]=='\r')) {
	            	if(g_onoff) maesure_onoff(0);
	            	else 		maesure_onoff(1);

	                TransUartBuf[0] = 0x31;  //Echo back
	                if(g_onoff) TransUartBuf[1] = '1';
	                else 		TransUartBuf[1] = '0';

					TransUartBuf[2] = '\r';
					TransUartBuf[3] = '\0';
					uart_put_str(TransUartBuf);
	            }
                break;
            case 0x32:  //Toggle Start/Stop
            	if((g_btready)&&(RcvUartBuf[1]=='\r')) {
	            	if(g_onoff) {
						if(g_state == MAESURE_PLAY) 	g_state=MAESURE_PAUSE;
						else if((g_state == MAESURE_STOP)||(g_state == MAESURE_PAUSE))	g_state = MAESURE_START;
					}
	                TransUartBuf[0] = 0x32;  //Echo back
	                if(g_state == MAESURE_PLAY) TransUartBuf[1] = '1';
	                else 						TransUartBuf[1] = '0';
            ...
            case 0x33:  //Get state
            	if((g_btready)&&(RcvUartBuf[1]=='\r')) {
	                TransUartBuf[0] = 0x33;  //Echo back
	                if(g_state == MAESURE_PLAY) TransUartBuf[1] = '1';
	                else 						TransUartBuf[1] = '0';
	
	                if(g_onoff) TransUartBuf[2] = '1';
	                else 		TransUartBuf[2] = '0';
                ...
                break;
            case 0x34: // read version info
            	if((g_btready)&&(RcvUartBuf[1]=='\r')) {
					TransUartBuf[0] = 0x34; //Echo back
					memcpy((void*)&TransUartBuf[1], (void*)&version[0], 16);
                ...
                break;
            case 0x35:	// Read POT command.
            	if((g_btready)&&(RcvUartBuf[1]=='\r')) {
					TransUartBuf[0] = 0x35;  //Echo back
					memcpy((void*)&TransUartBuf[1], (void*)&line1_buffer[0], 16);
					memcpy((void*)&TransUartBuf[17], (void*)&line2_buffer[0], 16);
...
void UserInit(void)
{
	...
	// Initialize LCD Port D7-D4 Output & Control Line ALL output
	mInitLCDPort();
    ...
    //Initialize PWM for LCD contrast
	LCD_PWM_Init();
        ...
        switch(ReceivedDataBuffer[0])				//Look at the data the host sent, to see what kind of application specific command it sent.
        {
            ...
            case 0x81:  //Get push button state
                ...
                if(!HIDTxHandleBusy(USBInHandle))
                {
                    ...
                    if(g_state == MAESURE_PLAY) ToSendDataBuffer[1] = 0x01;
                    else 						ToSendDataBuffer[1] = 0x00;

                    if(g_onoff) ToSendDataBuffer[2] = 0x01;
                    else 		ToSendDataBuffer[2] = 0x00;
            ...
            case 0x83: // read version info
                if(!HIDTxHandleBusy(USBInHandle))
                {
					...
					memcpy((void*)&ToSendDataBuffer[1], (void*)&version[0], 16);
            ...
            case 0x37:	//Read POT command.  Uses ADC to measure an analog voltage on one of the ANxx I/O pins, and returns the result to the host
                if(!HIDTxHandleBusy(USBInHandle))
                {
					...
					memcpy((void*)&ToSendDataBuffer[1], (void*)&line1_buffer[0], 16);
					memcpy((void*)&ToSendDataBuffer[17], (void*)&line2_buffer[0], 16);
